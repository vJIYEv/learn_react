...
function Header(props) {
  return <header>
    <h1><a href="/" onClick={function(event){  // 함수 표현식
...
function Nav(props) {
  ...
  for (let i=0; i<props.topics.length; i++) {
    ...
    lis.push(<li key={t.id}>
      <a id={t.id} href={'/read/'+t.id} onClick={function(event) {  // 함수 표현식
...
function App() {
  ...
  return (
    <div>
      <Header title="WEB" onChangeMode={function() {  // 함수표현식
        alert('Header');
      }}></Header>
      <Nav topics={topics} onChangeMode={function(id) {  // 함수표현식
        alert(id);
      }}></Nav>
